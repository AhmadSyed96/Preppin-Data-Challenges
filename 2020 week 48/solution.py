...
df_T2Stands = pd.read_excel('2020W48 Input.xlsx', sheet_name='Remote Stands Accesibility')\
                .assign(Gate = lambda dfx : dfx['Gate'].str[-1], Bus = 'Y')\
                .rename(columns={'Bus':'Requires Bus?'})
df_Stand_Loc = pd.read_excel('2020W48 Input.xlsx', sheet_name=2)\
...
df_output = df_Stand_Loc.merge(df_stands, how='left', on='Stand')\
                        .merge(df_T2Stands, how='left', on=['Gate', 'Requires Bus?'])\
