...
df = pd.read_excel('Transactions (1).xlsx').assign(Items_List = lambda dfx : dfx['Items'].str.split(', '))\
                                            .explode('Items_List').rename(columns={'Items_List' : 'Item'})
print(df)
df_expanded = pd.merge(df, df, on='TransactionID').query('Item_x != Item_y')[['TransactionID','Items_x', 'Item_x', 'Item_y']]\
