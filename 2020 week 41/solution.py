...
df_output_1 =  pd.merge(df_comments,
                        df_comments.groupby('Who', as_index=False)['Time'].min()\
                                                .query('Who != "Carl Allchin"'),
                        on=['Who', 'Time'])[['Who' ,'Comment', 'Region']].assign(City = lambda dfx : dfx['Comment'].str.title().str.extract('(.*)\,.*'), Country = lambda dfx : dfx['Comment'].str.title().str.extract('.*\,\s(\w*)\.?.*?'), first_time = lambda dfx : dfx['Comment'].str.contains('first time', case=False)).drop(columns=['Comment'])
# show(df_output_1)
